...
2. conda를 이용한 가상 독립환경 구성 (VMWare를 통한 우분투 설치와 비슷한 원리) 주로 딥러닝에 사용되는 Python library
...
* Keras 설치 실패시 방법
  [시작 -> Anaconda Navigator -> 좌측 메뉴 Environments -> machine 가상환경 선택 -> keras 검색 설치
  또는 cmd 관리자 모드로 실행.
